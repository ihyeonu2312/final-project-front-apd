...
const MyInquiries = () => {
  ...
  {inquiries.length === 0 ? (
    ...
    <div className="overflow-x-auto">
      <table className="w-full table-fixed border border-gray-400 border-collapse text-sm text-left">
        <thead className="bg-gray-100 text-gray-700">
          <tr>
            <th className="py-3 px-4 border border-gray-300 w-1/2">제목</th>
            <th className="py-3 px-4 border border-gray-300 w-1/4">작성일</th>
            <th className="py-3 px-4 border border-gray-300 w-1/4">상태</th>
          </tr>
        </thead>
        <tbody>
          {inquiries.map((inquiry) => (
            <tr key={inquiry.inquiryId} className="hover:bg-gray-50">
              <td className="py-3 px-4 border border-gray-300 truncate">
                <Link
